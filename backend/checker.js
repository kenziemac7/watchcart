...
async function checkItem(item) {
  const result = await scrapeProduct(item.url, item.size);
  ...
  const priceDropped    = result.price != null && result.price <= item.target_price;
  const sizeAvailable   = result.sizeAvailable;

  let alerted = false;
  if (item.status === 'watching' && priceDropped && sizeAvailable) {
    log(`ALERT: "${item.product_name}" is $${result.price} (target $${item.target_price}), size "${item.size}" in stock`);
    ...
    db.prepare("UPDATE watched_items SET status = 'alerted' WHERE id = ?").run(item.id);
    alerted = true;
  }
...
async function runCheck() {
  ...
  const items = db.prepare("SELECT * FROM watched_items WHERE status = 'watching'").all();
