...
app.get("/items", (_req, res) => {
  const items = db
    .prepare("SELECT * FROM watched_items ORDER BY created_at DESC")
    .all();
  res.json(items);
});
...
app.post("/items", async (req, res) => {
  ...
  if (isNaN(targetPrice) || targetPrice <= 0) {
    return res
      .status(400)
      .json({ error: "target_price must be a positive number" });
  }

  const itemCount = db
    .prepare("SELECT COUNT(*) as count FROM watched_items")
    .get().count;
  if (itemCount >= 20) {
    return res
      .status(400)
      .json({
        error:
          "Limit reached: WatchCart supports a maximum of 20 watched items. Remove an item before adding a new one.",
      });
  }
  ...
  let scraped;
  try {
    scraped = await extractProduct(parsedUrl.href);
  } catch (err) {
  ...
  const item = db
    .prepare("SELECT * FROM watched_items WHERE id = ?")
...
app.delete("/items/:id", (req, res) => {
  const result = db
    .prepare("DELETE FROM watched_items WHERE id = ?")
    .run(req.params.id);
...
app.patch("/items/:id", (req, res) => {
  ...
  const result = db
    .prepare(`UPDATE watched_items SET ${fields.join(", ")} WHERE id = ?`)
    .run(...values);
